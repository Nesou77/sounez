...
export function BlogPostShell({
  ...
  excerpt: string;
  children: ReactNode;
  /** Slugs of tools most relevant to this article — surfaced as primary CTAs. */
  ctaTools?: string[];
}) {
    ...
    <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
      <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_280px]">
        <article>
          ...
          {/* Top-of-article tool CTA — pushes readers into tools immediately */}
            ...
            <aside className="my-8 rounded-2xl border border-primary/30 bg-gradient-soft p-5">
              ...
              <div className="mt-4 grid gap-2">
                {primaryTools.map((t) => {
                  ...
                  return (
                    <a key={t.slug} href={`/${t.slug}`} className="group flex items-center gap-3 rounded-xl bg-card/80 p-3 transition hover:bg-card hover:shadow-soft">
        ...
        {/* Sidebar — desktop only, packed with internal links + ad */}
          ...
          <div className="sticky top-24 space-y-6">
            <div className="rounded-2xl border border-border bg-card p-5">
              ...
              <ul className="mt-3 space-y-1.5">
                {sidebarTools.map((t) => {
                  ...
                  return (
                    <li key={t.slug}>
                      <a href={`/${t.slug}`} className="group flex items-center gap-2.5 rounded-lg p-2 text-sm transition hover:bg-muted">
              ...
              <Link to="/tools" className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-primary hover:underline">
            ...
            <div className="rounded-2xl border border-border bg-gradient-soft p-5">
              ...
              <ul className="mt-3 space-y-2">
                {others.map((p) => (
                  <li key={p.slug}>
                    <a href={`/blog/${p.slug}`} className="text-sm font-medium leading-snug text-foreground/85 transition hover:text-primary">
                      {p.title}
                    </a>
                  </li>
                ))}
