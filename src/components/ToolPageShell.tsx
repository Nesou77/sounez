  ...
  return (
    <article className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
      <nav className="mb-6 text-xs text-muted-foreground">
        ...
        <a href={`/categories/${tool.category}`} className="hover:text-foreground capitalize">{tool.category.replace("-", " ")}</a> /{" "}
      ...
      <section className="my-12">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {moreTools.map((t) => {
            ...
            return (
              <a key={t.slug} href={`/${t.slug}`} className="group flex items-center gap-3 rounded-xl border border-border bg-card p-4 transition hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-soft">
        ...
        <div className="mt-6 text-sm">
          See all <Link to="/tools" className="font-medium text-primary hover:underline">100+ free tools</Link> or browse{" "}
          <Link to="/categories" className="font-medium text-primary hover:underline">all categories</Link>.
        </div>
      </section>
    </article>
  );
