      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-14 sm:px-6 md:grid-cols-12">
        ...
        <div className="md:col-span-3">
          ...
          <ul className="space-y-2.5 text-sm text-muted-foreground">
            {popular.map((t) => (
              <li key={t.slug}><a href={`/${t.slug}`} className="transition hover:text-foreground">{t.name}</a></li>
            ))}
          </ul>
        </div>
        ...
        <div className="md:col-span-2">
          ...
          <ul className="space-y-2.5 text-sm text-muted-foreground">
            {CATEGORIES.map((c) => (
              <li key={c.slug}><a href={`/categories/${c.slug}`} className="transition hover:text-foreground">{c.name}</a></li>
            ))}
            <li><Link to="/tools" className="transition hover:text-foreground">All Tools</Link></li>
          </ul>
        </div>
