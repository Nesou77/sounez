...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Free, fast and simple online tools — QR codes, password generator, image compressor, color palettes, hashtag and YouTube tools. No signup needed." },
      { property: "og:title", content: "Sounez — Free Online Tools" },
      { property: "og:description", content: "Simple Tools. Powerful Results. Free, fast tools for creators, designers and productivity." },
    ],
  }),
  component: Home,
});
    ...
    <>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="mx-auto max-w-5xl px-4 py-24 text-center sm:px-6 sm:py-32">
          ...
          <div className="animate-slide-up delay-150 mt-9 flex flex-wrap items-center justify-center gap-3">
            <Link to="/tools" className="group inline-flex items-center gap-2 rounded-xl bg-gradient-brand px-6 py-3.5 text-sm font-semibold text-primary-foreground shadow-pop transition-all duration-300 hover:-translate-y-0.5 hover:shadow-glow active:translate-y-0">
              Explore all tools
            ...
            {q && (
              <div className="animate-fade-in mt-3 max-h-72 overflow-y-auto rounded-2xl border border-border bg-popover p-2 text-left shadow-pop">
                ...
                {filtered.map((t) => {
                  ...
                  return (
                    <a key={t.slug} href={`/${t.slug}`} className="flex items-center gap-3 rounded-lg p-2.5 text-sm transition hover:bg-accent">
      ...
      <section id="popular" className="mx-auto max-w-7xl px-4 py-20 sm:px-6">
        <div className="mb-10 flex items-end justify-between gap-4">
          ...
          <Link to="/tools" className="hidden text-sm font-medium text-primary hover:underline sm:inline">View all →</Link>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6">
        <div className="mb-10 flex items-end justify-between gap-4">
          ...
          <Link to="/blog" className="hidden text-sm font-medium text-primary hover:underline sm:inline">All posts →</Link>
        </div>
        <div className="grid gap-6 md:grid-cols-3">
          {BLOG_POSTS.slice(0, 3).map((p) => (
            <a key={p.slug} href={`/blog/${p.slug}`} className="group overflow-hidden rounded-2xl border border-border/70 bg-card shadow-soft transition-all duration-300 hover:-translate-y-1 hover:shadow-glow">
