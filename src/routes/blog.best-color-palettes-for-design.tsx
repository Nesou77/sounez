...
import inlinePalette from "@/assets/blog/inline-palette-grid.jpg";

export const Route = createFileRoute("/blog/best-color-palettes-for-design")({
  head: () => ({
    meta: [
      { title: "The Best Color Palettes for Modern Design (2025) | Sounez" },
      { name: "description", content: "The principles behind color palettes that just work — for landing pages, apps and brands. Plus the free tools to build your own." },
      { property: "og:title", content: "Best Color Palettes for Modern Design" },
      { property: "og:description", content: "Palettes that just work — and the principles behind them." },
    ],
  }),
  component: () => (
    <BlogPostShell
    ...
    >
      ...
      <BlogImage src={inlinePalette} alt="A row of harmonious color swatches applied to a UI mockup" caption="Restraint reads as confidence." />

      <h2>The 60-30-10 rule (and why it works)</h2>
      <p>
        Almost every great interface follows the same proportional split:
      </p>
      <ul>
      ...
      <p>
        Body text must clear WCAG AA contrast (4.5:1 for normal text, 3:1 for large text). If a color
        fails, darken or lighten it by one step. Designers often skip this and end up with
        beautiful-but-illegible interfaces — the worst possible outcome.
      </p>
      ...
      <h2>How to use color across an entire product</h2>
      <p>
        Stop picking colors per page. Define them once as <em>semantic tokens</em>:
      </p>
      <ul>
        <li><code>--background</code> — the canvas</li>
        <li><code>--foreground</code> — primary text</li>
        <li><code>--primary</code> — your brand accent</li>
        <li><code>--muted</code> — backgrounds for cards and panels</li>
        <li><code>--border</code> — separators and outlines</li>
        <li><code>--destructive</code> — errors and warnings</li>
      </ul>
      <p>
        Apply tokens, never raw hex codes, in your components. Switching to dark mode then becomes a
        single CSS variable change instead of 200 component rewrites.
      </p>
      ...
      <p>
        One primary, one accent, plus 5–7 neutrals (background, surface, border, text variants).
        That's it. More and you'll lose consistency.
      </p>

      <h3>Should I use trendy colors?</h3>
      <p>
        Trends are great for hero gradients and marketing pages, but your core brand color should
        outlast trends. Pick something you'd be happy with in five years.
      </p>

      <h3>What's the easiest way to make a palette feel premium?</h3>
      <p>
        Reduce saturation and increase contrast. The most "premium" palettes are also the most
        restrained.
      </p>

      <h3>Light mode or dark mode first?</h3>
      <p>
        Design both from day one. Use semantic tokens so switching is one variable change, not a
        full rebuild.
      </p>

      <h3>Where do I get inspiration without copying?</h3>
      <p>
        Look outside web design — fashion lookbooks, film stills, museum exhibits. Then translate
        the palette into UI with the <a href="/color-palette-generator">Color Palette Generator</a>.
      </p>
      ...
      <p>
        ...
        right now — the rest of your design system will fall into place. For more design tools, browse{" "}
        <a href="/categories/design-tools">all design tools</a> or read{" "}
        <a href="/blog/best-free-tools-for-creators">our 10 best free creator tools</a>.
      </p>
    </BlogPostShell>
  ),
});
